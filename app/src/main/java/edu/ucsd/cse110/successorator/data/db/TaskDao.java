...
public interface TaskDao {
    ...
    default int append(TaskEntity task) {
        var newTask = new TaskEntity(1 + getMaxId(), task.description, task.dateCreated, task.isCompleted);
        return Math.toIntExact(insert(newTask));
    }

    @Transaction
    default int prepend(TaskEntity task) {
        var newTask = new TaskEntity(getMinId() - 1, task.description, task.dateCreated, task.isCompleted);
        return Math.toIntExact(insert(newTask));
    }
